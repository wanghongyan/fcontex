...
<title>分组添加 - <?php echo SYSTEM_NAME.' '.SYSTEM_VERSION; ?></title>
...
function group(mode, args)
{
	switch (mode)
	{
		case 'group.insert':
			...
			{
				top.naviSwitcher('user/mode=group.insert', 'user/mode=group.select');
			});
			break;
		default:
...
<form id="form_detail" name="form_detail" method="post" action="###" onsubmit="return group('group.insert');">
<table class="table_form" width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<th>分组名称</th>
		<td><input class="text" type="text" name="gr_name" id="gr_name" /> <span><cite>*</cite> 必填</span></td>
	</tr>
	<tr>
		<th>分组描述</th>
	...
	<tr>
		...
		<td>
			...
			<table id="rights_<?php echo $name; ?>" class="rights" border="0" cellspacing="0" cellpadding="0">
				...
				?>
				<tr>
					<td width="120"><input class="checkbox" type="checkbox" name="gr_rights[]" id="<?php echo $rights['key']; ?>" value="<?php echo $rights['key']; ?>" /><label for="<?php echo $rights['key']; ?>"><?php echo $rights['value']; ?></label></td>
					<?php
					...
					{
					?>
					<td><input class="checkbox" type="checkbox" name="gr_rights[]" id="<?php echo $rights['key']; ?>" value="<?php echo $rights['key']; ?>" /><label for="<?php echo $rights['key']; ?>"><?php echo $rights['value']; ?></label></td>
					<?php
					}
					else
					{
	...
	<tr class="action"><th>&nbsp;</th><td><input type="submit" class="button" value="确认添加" /><input type="button" class="button cancle" value="放弃添加" onclick="$$.confirm({text:'放弃添加并关闭窗口？', ok:function(){top.naviShut('user/mode=group.insert');}, icon:'WAR'});" /></td></tr>
</table>
</form>
